namespace UnitTests
{
    ...
    public class UnitTests
    {
        ...
        {
            TestSet<MyPoint> set1 = new TestSet<MyPoint>();
            set1.AddItem(new MyPoint(0, 0));
            set1.AddItem(new MyPoint(1, 1));
            set1.AddItem(new MyPoint(2, 2));
            set1.AddItem(new MyPoint(5, 0));
            set1.AddItem(new MyPoint(6, 1));
            set1.AddItem(new MyPoint(7, 2));
            set1.AddItem(new MyPoint(8, 5));
        ...
        public void TestPointSet()
        {
            ...
            Assert.True(set1.Points[0] == pSet.GetItem(set1, 0), "Check 0");
            Assert.True(set1.Points[3] == pSet.GetItem(set1, 1), "Check 1");
            Assert.True(set1.Points[4] == pSet.GetItem(set1, 2), "Check 2");
            Assert.True(set1.Points[6] == pSet.GetItem(set1, 3), "Check 3");
        }
        ...
        public void Test1()
        {
            TestSet<MyPoint> set1 = new TestSet<MyPoint>();
            set1.AddItem(new MyPoint(0, 0));
            set1.AddItem(new MyPoint(0, 1));
            set1.AddItem(new MyPoint(1, 0));
        ...
        {
            TestSet<MyPoint> set1 = new TestSet<MyPoint>();
            set1.AddItem(new MyPoint(1, 6));
            set1.AddItem(new MyPoint(2, 5));
            set1.AddItem(new MyPoint(2, 6));
            set1.AddItem(new MyPoint(3, 4));
            set1.AddItem(new MyPoint(3, 5));
            set1.AddItem(new MyPoint(3, 6));
            set1.AddItem(new MyPoint(3, 7));
            set1.AddItem(new MyPoint(5, 5));
            set1.AddItem(new MyPoint(5, 6));
            set1.AddItem(new MyPoint(6, 4));
        ...
        public void SpeedTest1()
        {
            for (int i = 0; i < 1000000; ++i)
            {
                ...
                currentLine.AddValidPoint(set1.Points[0]);
                currentLine.AddValidPoint(set1.Points[1]);

                TestSet<MyPoint> workSet = set1.Clone();
        ...
        public void SpeedTest2()
        {
            ...
            for (int i = 0; i < 100000; ++i)
            {
                ...
                currentLine.Add(0);
                currentLine.Add(1);
                LineParams par = new LineParams(set1.Points[0], set1.Points[1]);
                ...
                for (int k = 0; k < workSet.Count; ++k)
                {
                    ...
                    if (par.IsCollinear(set1.Points[id]))
                    {
                        currentLine.Add(id);
                        workSet.RemoveAt(id);
                    }
        ...
        public void OutputResults2(TestSet<MyPoint> set1, ResultCollection2 res)
        {
            int count = 0;
            foreach (Results2 r in res.ResultSet)
            {
                Console.WriteLine("Result set {0}", ++count);
                Console.WriteLine("Permutuations: {0}", r.Permutations);
                int c = r.Lines.Count;
